...
class Extractor(BaseETL):
    ...
    def _process_country(self, country_name, country_data):
        ...
        while current_date <= self.end_date:
            ...
            import_log_id = db.get_import_id_if_backfill_date_exists(current_date, country_id, self.api_id)
            if import_log_id:
                self.logger.info(f"Found existing import log (ID: {import_log_id}) for {country_name} on {date_str}, updating.")
                db.update_import_logs((self.timestamp, folder_path, file_name, import_time, import_log_id))
            else:
            ...
            try:
                ...
                if result:
                    ...
                    self.logger.info(f"Data collected and written to file {file_path} for {country_name} on {date_str}")
