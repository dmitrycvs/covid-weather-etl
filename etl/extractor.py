...
class Extractor(BaseETL):
    ...
    def _process_country(self, country_name, country_data):
        ...
        file_name = f"{country_data['iso']}_{self.api_type}_MONTHLY"
        ...
        while current_date <= self.end_date:
            ...
            import_log_id = db.get_import_id_if_backfill_date_exists(current_date, country_id)
            if import_log_id:
                self.logger.info(f"Found existing import log (ID: {import_log_id}) for {country_name} on {date_str}, updating.")
                db.update_import_logs((folder_path, import_time, import_log_id))
            else:
                import_log_id = db.insert_import_log((country_id, self.timestamp, folder_path, file_name, import_time, import_time, current_date))
                self.logger.info(f"Created new import log (ID: {import_log_id}) for {country_name} on {date_str}")
            ...
            try:
                ...
                self.logger.debug(f"Received response for {country_name} on {date_str} (Status: {response.status_code})")

                if result:
                    data.append([result])
                    self.logger.info(f"Data collected for {country_name} on {date_str}")

                db.insert_api_import_log((self.api_id, import_log_id, start_time, end_time, response.status_code, None))

            except requests.exceptions.HTTPError as e:
                error_msg = f"HTTP Error {e.response.status_code} for {country_name} on {date_str}: {str(e)}"
                self.logger.error(error_msg)
                db.insert_api_import_log((self.api_id, import_log_id, start_time, datetime.now(), e.response.status_code, str(e)))
        ...
        if data:
            ...
            self.logger.warning(f"No data collected for {country_name} between {self.start_date} and {self.end_date}")
    ...
    def run(self):
        ...
        for country, info in countries.items():
            ...
            file_path = self._process_country(country, info)
            file_paths.append(file_path)
            self.logger.info(f"Finished extraction for {country}")

        self.logger.info(f"Extraction complete. {len(file_paths)} files saved.")
        return file_paths
